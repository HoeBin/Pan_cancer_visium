...
# Code/ 바로 아래의 FigureN.R은 source(file.path(root_dir, "common/config.R"))로 이 파일을 불러온다.
# Code/FigureN/0X_*.R처럼 한 단계 더 깊은 스크립트는 소싱 전에 code_dir(=Code/)를 직접 정의해 둔다 —
# commandArgs()는 항상 최상위로 실행된 스크립트의 경로만 알려주므로, 깊이가 다르면 아래 dirname()
# 계산이 스크립트 위치와 어긋난다.
